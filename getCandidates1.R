...
getCandidates2  <-   function(signo) {
  ...
  if (signo==1) {   #get err rows like '*bike', w. balance<0
    ...
    col2A <- intersect(grep(pattern=paste('\\b',genericA,'\\b',sep=''),err[,1],value=T), err[which(err[,2] > 0),1] )
  ...
  if (signo==1) {   #err rows type 'car', balance < 0
    ...
    col2B <- intersect(grep(pattern=paste('\\b',genericB,'\\b',sep=''),err[,1],value=T), err[which(err[,2] < 0),1]  )
